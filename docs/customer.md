...
$request = new Request\ListAccountBalances(
    ...
    [
        'page' => 1485,                           // ページ番号
        'per_page' => 9692,                       // 1ページ分の取引数
        'expires_at_from' => "2022-09-07T20:53:02.000000Z", // 有効期限の期間によるフィルター(開始時点)
        'expires_at_to' => "2021-01-06T02:09:03.000000Z", // 有効期限の期間によるフィルター(終了時点)
...
$request = new Request\ListAccountExpiredBalances(
    ...
    [
        'page' => 1606,                           // ページ番号
        'per_page' => 2874,                       // 1ページ分の取引数
        'expires_at_from' => "2023-10-02T12:45:35.000000Z", // 有効期限の期間によるフィルター(開始時点)
        'expires_at_to' => "2023-07-12T02:40:43.000000Z", // 有効期限の期間によるフィルター(終了時点)
        'direction' => "asc"                      // 有効期限によるソート順序
    ]
);
```
...
$request = new Request\UpdateCustomerAccount(
    ...
    [
        ...
        'account_name' => "9S3Zg4O5dK9OBTn3gY0HIwJr5Xn6R9PIw5eC52tvIBnMyMg4CnT2dj7ORUTt4jEgn4792da7QYy7V605lzcBixerwgOsZo2yFQXiifPwyEPkMTjwK5UmBamQcUvvHD25XYGaGoRmlkWpVKSQYACWhdJgT5oXIAxp1c5Q2vG7By91KC2xkwbMvROWfUAhh6XnZz0yJYgRGAM6oTzljbZYS9b6qmrSFaDiVxdn1z0TuA7dLQ8GnuuGnm3", // アカウント名
        'external_id' => "0ZKYlqHYAPfacx4ba4pxXiFCicQd3QQrdtpp5IlW8KnTar", // 外部ID
...
$request = new Request\GetCustomerAccounts(
    ...
    [
        'page' => 4643,                           // ページ番号
        'per_page' => 1937,                       // 1ページ分のウォレット数
        'created_at_from' => "2022-11-30T12:50:25.000000Z", // ウォレット作成日によるフィルター(開始時点)
        'created_at_to' => "2022-12-29T12:40:52.000000Z", // ウォレット作成日によるフィルター(終了時点)
        'is_suspended' => FALSE,                  // ウォレットが凍結状態かどうかでフィルターする
        'status' => "active",                     // ウォレット状態
        'external_id' => "01ZxeZpTa0FFkkUFLVCDKp9TvCsVFg3Dy6t9F", // 外部ID
        'tel' => "006692195224",                  // エンドユーザーの電話番号
        'email' => "I5NM6J7Ehk@zGk2.com"          // エンドユーザーのメールアドレス
    ]
);
```
...
$request = new Request\CreateCustomerAccount(
    ...
    [
        ...
        'external_id' => "yYle2ZOPXJOiEYcNwwBKhoxCdqw8" // 外部ID
    ]
);
```
...
$request = new Request\GetShopAccounts(
    ...
    [
        'page' => 7215,                           // ページ番号
        'per_page' => 468,                        // 1ページ分のウォレット数
        'created_at_from' => "2021-03-10T16:39:32.000000Z", // ウォレット作成日によるフィルター(開始時点)
        'created_at_to' => "2024-12-02T14:12:12.000000Z", // ウォレット作成日によるフィルター(終了時点)
        'is_suspended' => FALSE                   // ウォレットが凍結状態かどうかでフィルターする
    ]
);
```
...
$request = new Request\ListCustomerTransactions(
    ...
    [
        ...
        'from' => "2020-07-16T04:17:19.000000Z",  // 開始日時
        'to' => "2020-05-20T05:22:42.000000Z",    // 終了日時
        'page' => 1,                              // ページ番号
