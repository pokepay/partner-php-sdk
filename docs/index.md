...
$request = new Request\GetCpmToken(
    "mq3NPeGSjNJPl3IkWmftNw"                      // cpmToken: CPMトークン
);
```
...
$request = new Request\ListTransactions(
    [
        'from' => "2020-04-07T18:50:01.000000+09:00", // 開始日時
        'to' => "2023-06-06T16:20:36.000000+09:00", // 終了日時
        'page' => 1,                              // ページ番号
...
$request = new Request\CreateTransaction(
    ...
    [
        'money_amount' => 2969,
        'point_amount' => 3284,
        'point_expires_at' => "2022-08-23T00:57:12.000000+09:00", // ポイント有効期限
        'description' => "k2lmmI8Ed7s7r6X05TVPEqyU8nmgr8tnMV2tpncMeLaj6Eg7Nnm9I7n5wJLFqEZhX6Iem1hniS6eTglBN8UnpYpRyEyQgVDg0hy"
    ]
);
```
...
$request = new Request\ListTransactionsV2(
    [
        ...
        'customer_name' => "太郎",                  // エンドユーザー名
        'description' => "店頭QRコードによる支払い",         // 取引説明文
        'transaction_id' => "a",                  // 取引ID
        'is_modified' => TRUE,                    // キャンセルフラグ
        'types' => ["topup", "payment"],          // 取引種別 (複数指定可)、チャージ=topup、支払い=payment
        'from' => "2022-08-15T14:38:48.000000+09:00", // 開始日時
        'to' => "2021-02-08T19:35:43.000000+09:00", // 終了日時
...
$request = new Request\CreateTopupTransaction(
    ...
    [
        ...
        'money_amount' => 6352,                   // マネー額
        'point_amount' => 6857,                   // ポイント額
        'point_expires_at' => "2021-10-28T19:11:05.000000+09:00", // ポイント有効期限
...
$request = new Request\CreateCpmTransaction(
    "rADOehKQdRcOXhQxc4PPxV",                     // cpmToken: CPMトークン
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",       // shopId: 店舗ID
    5969.0,                                       // amount: 取引金額
    [
...
$request = new Request\RefundTransaction(
    ...
    [
        ...
        'returning_point_expires_at' => "2021-04-20T01:28:56.000000+09:00" // 返却ポイントの有効期限
    ]
);
```
...
$request = new Request\GetAccountTransferSummary(
    ...
    [
        'from' => "2022-01-15T23:16:42.000000+09:00", // 集計期間の開始時刻
        'to' => "2023-06-15T11:44:22.000000+09:00", // 集計期間の終了時刻
...
$request = new Request\ListTransfers(
    [
        'from' => "2023-02-26T18:09:27.000000+09:00",
        'to' => "2023-11-01T17:13:00.000000+09:00",
        'page' => 6478,
        'per_page' => 6637,
        'shop_id' => "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
        'shop_name' => "OT5z5woCc8pEGWjmS0osyR2KF2P7",
        'customer_id' => "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
        'customer_name' => "vJ5vSh9EDNQnOLRlkak10UdVnAId59JeVIWApoE7ASo2Gb6Dm321lTOYH3jdMRUkL0Zx",
        'transaction_id' => "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
        'private_money_id' => "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
        'is_modified' => FALSE,
        'transaction_types' => ["exchange", "cashback", "topup"],
        'transfer_types' => ["payment", "coupon", "exchange"], // 取引明細の種類でフィルターします。
...
$request = new Request\ListTransfersV2(
    [
        'shop_id' => "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // 店舗ID
        'shop_name' => "dvIVIFuFFgH87Npf4V8FLO8eFvUA7RMtkRY85MHJOf6IX9yC8rniKyUmZ69MPGmala8R9YrMzkNwRhiUARrX3lhcxeZi0qCyU8o37589VTcSpJiAtUW1a6pkFXZnzXLQ4QIKUXI7a6DkF7Y6IGO52wpYNL1qX8GbNXEsgcyODgjJXJsvkaXr6xaPn5fj223dNO8HtR2goX6Pm", // 店舗名
        'customer_id' => "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // エンドユーザーID
        'customer_name' => "7DAkFPPeLimmPE0ZHmdgsZ7cyIf2baUhcbkA3cqEZLbFrIxZwzt749xWU8kJDQcKJRCdCMjvZaYHozfUSXmJDhvRQzxqiTmIaYWpMzjd4TcI8q8z67xr7kHPL5Hi6p1iqgkHrv6cFC69V5PBnvm83YnErfeiF6useFmUN3JCZ3PImi2wTVdKFFFCXfBJAR2BgMXyiurnuOg01A0dkCbFsIgxF8RpzgCYCjyBfcG", // エンドユーザー名
        ...
        'per_page' => 50,                         // 1ページ分の取引数
        'transfer_types' => ["campaign", "topup", "expire"], // 取引明細種別 (複数指定可)
        'description' => "店頭QRコードによる支払い",         // 取引詳細説明文
        'from' => "2022-08-18T16:36:52.000000+09:00", // 開始日時
        'to' => "2023-01-12T16:33:42.000000+09:00" // 終了日時
    ]
);
```
...
$request = new Request\CreateCheck(
    ...
    [
        'money_amount' => 6610.0,                 // 付与マネー額
        'point_amount' => 8989.0,                 // 付与ポイント額
        'description' => "test check",            // 説明文(アプリ上で取引の説明文として表示される)
        'is_onetime' => TRUE,                     // ワンタイムかどうかのフラグ
        'usage_limit' => 8976,                    // ワンタイムでない場合の最大読み取り回数
        'expires_at' => "2023-06-02T06:14:27.000000+09:00", // チャージQRコード自体の失効日時
        'point_expires_at' => "2021-06-29T13:17:04.000000+09:00", // チャージQRコードによって付与されるポイント残高の有効期限
...
$request = new Request\ListChecks(
    [
        ...
        'private_money_id' => "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // マネーID
        'organization_code' => "jy71IntHmaYrdJoPC41v8KrW2lOWd", // 組織コード
        'expires_from' => "2020-07-24T21:41:40.000000+09:00", // 有効期限の期間によるフィルター(開始時点)
        'expires_to' => "2020-10-19T21:15:30.000000+09:00", // 有効期限の期間によるフィルター(終了時点)
        'created_from' => "2022-08-10T09:08:14.000000+09:00", // 作成日時の期間によるフィルター(開始時点)
        'created_to' => "2023-10-12T14:08:35.000000+09:00", // 作成日時の期間によるフィルター(終了時点)
        'issuer_shop_id' => "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // 発行店舗ID
        'description' => "V",                     // チャージQRコードの説明文
...
$request = new Request\UpdateCheck(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",       // checkId: チャージQRコードのID
    [
        'money_amount' => 9654.0,                 // 付与マネー額
        'point_amount' => 1569.0,                 // 付与ポイント額
        'description' => "test check",            // チャージQRコードの説明文
        'is_onetime' => FALSE,                    // ワンタイムかどうかのフラグ
        'usage_limit' => 5592,                    // ワンタイムでない場合の最大読み取り回数
        'expires_at' => "2022-08-30T14:02:02.000000+09:00", // チャージQRコード自体の失効日時
        'point_expires_at' => "2020-01-31T22:33:35.000000+09:00", // チャージQRコードによって付与されるポイント残高の有効期限
        'point_expires_in_days' => 60,            // チャージQRコードによって付与されるポイント残高の有効期限(相対日数指定)
        'bear_point_account' => "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // ポイント額を負担する店舗のウォレットID
...
$request = new Request\ListBills(
    [
        'page' => 6855,                           // ページ番号
        'per_page' => 120,                        // 1ページの表示数
        'bill_id' => "xrc",                       // 支払いQRコードのID
        'private_money_id' => "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // マネーID
        'organization_code' => "14-5-R9FY",       // 組織コード
        'description' => "test bill",             // 取引説明文
        'created_from' => "2021-10-28T17:15:49.000000+09:00", // 作成日時(起点)
        'created_to' => "2022-11-30T12:51:27.000000+09:00", // 作成日時(終点)
        'shop_name' => "bill test shop1",         // 店舗名
        'shop_id' => "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // 店舗ID
        'lower_limit_amount' => 1284,             // 金額の範囲によるフィルタ(下限)
        'upper_limit_amount' => 1616,             // 金額の範囲によるフィルタ(上限)
...
$request = new Request\ListAccountBalances(
    ...
    [
        'page' => 7956,                           // ページ番号
        'per_page' => 8572,                       // 1ページ分の取引数
        'expires_at_from' => "2021-10-30T05:34:37.000000+09:00", // 有効期限の期間によるフィルター(開始時点)
        'expires_at_to' => "2021-02-19T09:53:35.000000+09:00", // 有効期限の期間によるフィルター(終了時点)
...
$request = new Request\ListAccountExpiredBalances(
    ...
    [
        'page' => 8091,                           // ページ番号
        'per_page' => 7613,                       // 1ページ分の取引数
        'expires_at_from' => "2022-12-22T06:32:10.000000+09:00", // 有効期限の期間によるフィルター(開始時点)
        'expires_at_to' => "2023-02-22T18:27:28.000000+09:00", // 有効期限の期間によるフィルター(終了時点)
        'direction' => "desc"                     // 有効期限によるソート順序
    ]
);
```
...
$request = new Request\UpdateCustomerAccount(
    ...
    [
        ...
        'account_name' => "MnHbebgx27cx4nnHSwdIgWqx8bObAQ09PnefS5j1KcYZH6kNX2tCYZfyQfNmJsa7CLdtgJafN6NUdJbvy2FlWDkJtqK3kayxzVhbgoN2mzq8b098jzs18YvxUrbqKAQduefJMhLLQ", // アカウント名
        'external_id' => "MvmOWqkZbMCQg2QUS89ibyhdG3tF7KFPc5udhFJMqccrWi0F0", // 外部ID
...
$request = new Request\GetCustomerAccounts(
    ...
    [
        'page' => 2657,                           // ページ番号
        'per_page' => 4676,                       // 1ページ分のウォレット数
        'created_at_from' => "2022-03-04T14:46:38.000000+09:00", // ウォレット作成日によるフィルター(開始時点)
        'created_at_to' => "2022-06-28T07:01:08.000000+09:00", // ウォレット作成日によるフィルター(終了時点)
        'is_suspended' => FALSE,                  // ウォレットが凍結状態かどうかでフィルターする
        'status' => "pre-closed",                 // ウォレット状態
        'external_id' => "RyKhz7Y7hCkXJ6yyJwjYX", // 外部ID
        'tel' => "0936-4372-5254",                // エンドユーザーの電話番号
        'email' => "fba7FJPOk7@aWxq.com"          // エンドユーザーのメールアドレス
    ]
);
```
...
$request = new Request\CreateCustomerAccount(
    ...
    [
        ...
        'external_id' => "GQhz5XjLGvju2"          // 外部ID
    ]
);
```
...
$request = new Request\GetShopAccounts(
    ...
    [
        'page' => 5850,                           // ページ番号
        'per_page' => 6167,                       // 1ページ分のウォレット数
        'created_at_from' => "2020-08-10T17:03:30.000000+09:00", // ウォレット作成日によるフィルター(開始時点)
        'created_at_to' => "2020-10-01T17:29:19.000000+09:00", // ウォレット作成日によるフィルター(終了時点)
        'is_suspended' => FALSE                   // ウォレットが凍結状態かどうかでフィルターする
    ]
);
```
...
$request = new Request\ListCustomerTransactions(
    ...
    [
        ...
        'from' => "2020-06-09T07:06:21.000000+09:00", // 開始日時
        'to' => "2022-07-01T09:39:06.000000+09:00", // 終了日時
        'page' => 1,                              // ページ番号
        'per_page' => 50                          // 1ページ分の取引数
    ]
);
...
$request = new Request\ListShops(
    [
        ...
        'private_money_id' => "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // マネーID
        'name' => "oxスーパー三田店",                    // 店舗名
        'postal_code' => "389-1255",              // 店舗の郵便番号
        'address' => "東京都港区芝...",                 // 店舗の住所
        'tel' => "07-6883-079",                   // 店舗の電話番号
        'email' => "r1xMVyCXOP@OanJ.com",         // 店舗のメールアドレス
        'external_id' => "4iB",                   // 店舗の外部ID
...
$request = new Request\CreateShop(
    ...
    [
        'shop_postal_code' => "826-4952",         // 店舗の郵便番号
        'shop_address' => "東京都港区芝...",            // 店舗の住所
        'shop_tel' => "0236-80-5136",             // 店舗の電話番号
        'shop_email' => "X5ss7yhIv8@Lgom.com",    // 店舗のメールアドレス
        'shop_external_id' => "WF",               // 店舗の外部ID
...
$request = new Request\CreateShopV2(
    "oxスーパー三田店",                                  // name: 店舗名
    [
        'postal_code' => "816-3339",              // 店舗の郵便番号
        'address' => "東京都港区芝...",                 // 店舗の住所
        'tel' => "014-38-5856",                   // 店舗の電話番号
        'email' => "FveMGDDWD7@O6Wu.com",         // 店舗のメールアドレス
        'external_id' => "Ts4gv6eXnMfNGg8fhxQ7",  // 店舗の外部ID
        'organization_code' => "ox-supermarket",  // 組織コード
...
$request = new Request\UpdateShop(
    ...
    [
        'name' => "oxスーパー三田店",                    // 店舗名
        'postal_code' => "030-7122",              // 店舗の郵便番号
        'address' => "東京都港区芝...",                 // 店舗の住所
        'tel' => "00659094401",                   // 店舗の電話番号
        'email' => "jLQPaC3Ksf@XNe1.com",         // 店舗のメールアドレス
        'external_id' => "gygE",                  // 店舗の外部ID
...
$request = new Request\CreateUserAccount(
    ...
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",       // privateMoneyId: マネーID
    [
        'name' => "RVsp3AxmR4KrtS8po0ZZi6ABdi09pYTULtQLqvaZpgkN6yKCkbsOpn0GBtVQ3V8SPITcDtPClfID8uPwdbaKTg7KLZeECBmO8lIxZxffiNO26mW", // ウォレット名
        'external_id' => "nNZ22umoTCfZcCpIAbgloZQS2irm6YNDK", // 外部ID
        'metadata' => "{\"key1\":\"foo\",\"key2\":\"bar\"}" // ウォレットに付加するメタデータ
    ]
);
```
...
$request = new Request\GetPrivateMoneyOrganizationSummaries(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",       // privateMoneyId: マネーID
    [
        'from' => "2022-04-15T11:17:54.000000+09:00", // 開始日時(toと同時に指定する必要有)
        'to' => "2023-10-28T00:04:53.000000+09:00", // 終了日時(fromと同時に指定する必要有)
        'page' => 1,                              // ページ番号
        'per_page' => 50                          // 1ページ分の取引数
    ]
);
...
$request = new Request\GetPrivateMoneySummary(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",       // privateMoneyId: マネーID
    [
        'from' => "2021-12-05T12:57:48.000000+09:00", // 開始日時
        'to' => "2021-04-05T10:59:37.000000+09:00" // 終了日時
    ]
);
```
...
$request = new Request\BulkCreateTransaction(
    "ddy5pLVFl21CdCa",                            // name: 一括取引タスク名
    "YxG",                                        // content: 取引する情報のCSV
    "Khixmc3fIHHfhjZoOjF3FdjrY4gPDA17Wzbd",       // requestId: リクエストID
    [
        'description' => "0M2iQVGHJJtyXR0ZMWs2cHp3UWhyNJSXEQQMmBUIEOp46ijevmqEck7fgAot", // 一括取引の説明
        'private_money_id' => "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" // マネーID
    ]
);
```
...
$request = new Request\CreateCampaign(
    "5RFqM1jAFvVRqoCCL3XTTq0lHm5zn7FVkvVdA00Sr0dI7apPREwiHGiDPAsmzp3Xpc7DFLgNY9RzLE8HaRJLtDlYh9hld6nahuQqK3", // name: キャンペーン名
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",       // privateMoneyId: マネーID
    "2022-11-09T14:31:07.000000+09:00",           // startsAt: キャンペーン開始日時
    "2023-04-16T07:08:48.000000+09:00",           // endsAt: キャンペーン終了日時
    9978,                                         // priority: キャンペーンの適用優先度
    "topup",                                      // event: イベント種別
    [
        'bear_point_shop_id' => "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // ポイント負担先店舗ID
        'description' => "BcG2cdFIxlBavkOlJWLvBerqW2vRDSFddg8FgAKsuOIFneuQvrGNmFFjBpQ16iAIL2vXy3BeYg2Hb8rsTwhxD5QHhkZKjcmSrouERjQTug0bq1V", // キャンペーンの説明文
        'status' => "enabled",                    // キャンペーン作成時の状態
        'point_expires_at' => "2020-08-26T22:34:18.000000+09:00", // ポイント有効期限(絶対日時指定)
        'point_expires_in_days' => 9567,          // ポイント有効期限(相対日数指定)
...
, ["point_amount" => 5, "point_amount_unit" => "percent", "subject_more_than_or_equal" => 1000, "subject_less_than" => 5000]
        ...
        'minimum_number_for_combination_purchase' => 7405, // 複数種類の商品を同時購入するときの商品種別数の下限
        'exist_in_each_product_groups' => FALSE,  // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
        'max_point_amount' => 4515,               // キャンペーンによって付与されるポイントの上限
        'max_total_point_amount' => 2338,         // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
...
$request = new Request\ListCampaigns(
    ...
    [
        ...
        'available_from' => "2020-10-25T21:08:03.000000+09:00", // 指定された日時以降に適用可能期間が含まれているか
        'available_to' => "2020-01-09T02:50:04.000000+09:00", // 指定された日時以前に適用可能期間が含まれているか
        'page' => 1,                              // ページ番号
...
$request = new Request\UpdateCampaign(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",       // campaignId: キャンペーンID
    [
        'name' => "HxUBcknLOxhIMo9FzDj31LOww7pPUbTIEbU7t9ak4tLMhSCN1dLufqIaq9VPZZcaxGHdqDxpHv2rrXeDHksE3egtrpFiCcoIENv1FNXfWHeuzkZQpRI7lkkRZic2G2ZBJ7VxnuYxQLXM3ULhLaDQIZPLWOSTxiuGfyEnhDlZe306zyRiggMAbDct2Tc9Ew4vhdkl2CqDs2FpSU7DRyfsVjoTvIr0yy6ZUZKy3eLuUBKBEThM4qULaICy8", // キャンペーン名
        'starts_at' => "2022-10-12T20:26:41.000000+09:00", // キャンペーン開始日時
        'ends_at' => "2023-03-20T16:36:07.000000+09:00", // キャンペーン終了日時
        'priority' => 1175,                       // キャンペーンの適用優先度
        'event' => "topup",                       // イベント種別
        'description' => "T77vIZIOwmTsNLjmoaN9EjEZqCNPFkEA1bTr9cUa91Tf5GZnSgQ3EubsJLK8Um6mUpnjjvpN8q7uPu2SSxrWQeXIsIjuZV6cnJ0733A244SAHQtqNg", // キャンペーンの説明文
        'status' => "disabled",                   // キャンペーン作成時の状態
        'point_expires_at' => "2020-05-03T05:33:55.000000+09:00", // ポイント有効期限(絶対日時指定)
        'point_expires_in_days' => 2474,          // ポイント有効期限(相対日数指定)
        'is_exclusive' => TRUE,                   // キャンペーンの重複設定
        'subject' => "all",                       // ポイント付与の対象金額の種別
        'amount_based_point_rules' => [["point_amount" => 5, "point_amount_unit" => "percent", "subject_more_than_or_equal" => 1000, "subject_less_than" => 5000]
, ["point_amount" => 5, "point_amount_unit" => "percent", "subject_more_than_or_equal" => 1000, "subject_less_than" => 5000]
...
, ["from" => "12:00", "to" => "23:59"]
        ...
        'minimum_number_for_combination_purchase' => 4870, // 複数種類の商品を同時購入するときの商品種別数の下限
        'exist_in_each_product_groups' => FALSE,  // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
        'max_point_amount' => 5071,               // キャンペーンによって付与されるポイントの上限
        'max_total_point_amount' => 2000,         // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
        'applicable_account_metadata' => ["key" => "sex", "value" => "male"]
                                                  // ウォレットに紐付くメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
    ]
);
...
$request = new Request\ListCoupons(
    ...
    [
        'coupon_id' => "nz0l7",                   // クーポンID
        'coupon_name' => "KOAPeT",                // クーポン名
        'issued_shop_name' => "ymM",              // 発行店舗名
        'available_shop_name' => "WLnXdi4LMg",    // 利用可能店舗名
        'available_from' => "2023-08-27T13:51:29.000000+09:00", // 利用可能期間 (開始日時)
        'available_to' => "2022-02-02T14:41:38.000000+09:00", // 利用可能期間 (終了日時)
        'page' => 1,                              // ページ番号
        'per_page' => 50                          // 1ページ分の取得数
    ]
);
...
$request = new Request\CreateCoupon(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
    "Lzotf7NqBARSCikx6UYhQeGMkeRiCYcoRDXTsPVJwnN2rF93lIuvSHta7GSQZVx",
    "2022-11-07T22:34:51.000000+09:00",
    "2021-06-18T10:52:42.000000+09:00",
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",       // issuedShopId: 発行元の店舗ID
    [
        'description' => "KR894lZ9VSDqzw1PIiG1hENZ6vYGmJyJOrpsfnMGyPjhGHLYlN",
        'discount_amount' => 9652,
        'discount_percentage' => 3000.0,
        'discount_upper_limit' => 3588,
        'display_starts_at' => "2020-08-13T18:43:55.000000+09:00", // クーポンの掲載期間(開始日時)
        'display_ends_at' => "2020-06-24T19:11:02.000000+09:00", // クーポンの掲載期間(終了日時)
        'is_disabled' => TRUE,                    // 無効化フラグ
        'is_hidden' => FALSE,                     // クーポン一覧に掲載されるかどうか
        'is_public' => FALSE,                     // アプリ配信なしで受け取れるかどうか
        'code' => "j",                            // クーポン受け取りコード
        'usage_limit' => 8491,                    // ユーザごとの利用可能回数(NULLの場合は無制限)
        'min_amount' => 224,                      // クーポン適用可能な最小取引額
...
$request = new Request\UpdateCoupon(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",       // couponId: クーポンID
    [
        'name' => "XKpCCUYwKr7tPXqAbYyB",
        'description' => "isKhueiIE2AygZ",
        'discount_amount' => 5585,
        'discount_percentage' => 5985.0,
        'discount_upper_limit' => 8689,
        'starts_at' => "2020-08-17T03:14:21.000000+09:00",
        'ends_at' => "2020-12-26T08:11:05.000000+09:00",
        'display_starts_at' => "2023-09-28T05:39:47.000000+09:00", // クーポンの掲載期間(開始日時)
        'display_ends_at' => "2023-05-12T19:57:27.000000+09:00", // クーポンの掲載期間(終了日時)
        'is_disabled' => TRUE,                    // 無効化フラグ
        'is_hidden' => TRUE,                      // クーポン一覧に掲載されるかどうか
        'is_public' => FALSE,                     // アプリ配信なしで受け取れるかどうか
        'code' => "Cj3PoWn0E",                    // クーポン受け取りコード
        'usage_limit' => 9000,                    // ユーザごとの利用可能回数(NULLの場合は無制限)
        'min_amount' => 3724,                     // クーポン適用可能な最小取引額
