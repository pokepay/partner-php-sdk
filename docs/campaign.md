...
$request = new Request\ListCampaigns(
    ...
    [
        ...
        'available_from' => "2024-04-19T01:56:08.000000Z", // 指定された日時以降に適用可能期間が含まれているか
        'available_to' => "2020-07-17T12:10:08.000000Z", // 指定された日時以前に適用可能期間が含まれているか
        'page' => 1,                              // ページ番号
...
$request = new Request\CreateCampaign(
    "mJwg74tJdyJ5Llab29gfUQ6hTQL306",             // name: キャンペーン名
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",       // privateMoneyId: マネーID
    "2021-01-11T11:18:00.000000Z",                // startsAt: キャンペーン開始日時
    "2023-11-07T18:48:35.000000Z",                // endsAt: キャンペーン終了日時
    1609,                                         // priority: キャンペーンの適用優先度
    "topup",                                      // event: イベント種別
    [
        'bear_point_shop_id' => "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // ポイント負担先店舗ID
        'description' => "MLHDmfb2965KcWooPsLAa0LofoeILq2j1JbokM11iel9SifEKQQKEl5jTOY", // キャンペーンの説明文
        'status' => "enabled",                    // キャンペーン作成時の状態
        'point_expires_at' => "2021-05-31T09:35:01.000000Z", // ポイント有効期限(絶対日時指定)
        'point_expires_in_days' => 9782,          // ポイント有効期限(相対日数指定)
        ...
        'minimum_number_of_products' => 6494,     // キャンペーンを適用する1会計内の商品個数の下限
        'minimum_number_of_amount' => 3539,       // キャンペーンを適用する1会計内の商品総額の下限
        'minimum_number_for_combination_purchase' => 7634, // 複数種類の商品を同時購入するときの商品種別数の下限
        'exist_in_each_product_groups' => FALSE,  // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
        'max_point_amount' => 4392,               // キャンペーンによって付与されるポイントの上限
        'max_total_point_amount' => 9585,         // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
...
,                                                 // 取引時に指定するメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
        'budget_caps_amount' => 1369986438        // キャンペーン予算上限
    ]
);
```
...
$request = new Request\UpdateCampaign(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",       // campaignId: キャンペーンID
    [
        'name' => "66DYXbWwtCBK4yI7b7ruIn1DQefV0LKmn0D6u1aqXUgLXLPq2aRw08aQ0rfHosccmXhG1yeE5aq4GKVSCfP0aoPIG5NuiBMU7rfLf6FhpORYw57l88LjJn33RIRSOmlXSQfzzTwn3Dxt4Xew7YzDaZ1J9OdsQM2IVUV93tsgTE0JEew3ek7732woVpaWAn4e207OnXy1NWRJfp7ZK3W", // キャンペーン名
        'starts_at' => "2022-09-05T09:29:13.000000Z", // キャンペーン開始日時
        'ends_at' => "2022-10-15T22:04:01.000000Z", // キャンペーン終了日時
        'priority' => 8545,                       // キャンペーンの適用優先度
        'event' => "external-transaction",        // イベント種別
        'description' => "i0F0S2aIOKkN5iwpVUwFU1amkd1FBZBysFgH8TiyAaF4dUSAbqyi68iyJ302sQl233vCftoqwC5tymvF1K23X2uYu46ypSW9PxtiaID1SUCfz9yEelMoF", // キャンペーンの説明文
        'status' => "disabled",                   // キャンペーン作成時の状態
        'point_expires_at' => "2024-06-15T10:34:33.000000Z", // ポイント有効期限(絶対日時指定)
        'point_expires_in_days' => 3096,          // ポイント有効期限(相対日数指定)
...
, ["product_code" => "4912345678904", "classification_code" => "c123"]
        ...
        'blacklisted_shop_ids' => ["xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"], // キャンペーン適用対象外となる店舗IDのリスト(ブラックリスト方式)
        'minimum_number_of_products' => 1212,     // キャンペーンを適用する1会計内の商品個数の下限
        'minimum_number_of_amount' => 3117,       // キャンペーンを適用する1会計内の商品総額の下限
        'minimum_number_for_combination_purchase' => 6096, // 複数種類の商品を同時購入するときの商品種別数の下限
        'exist_in_each_product_groups' => TRUE,   // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
        'max_point_amount' => 2013,               // キャンペーンによって付与されるポイントの上限
        'max_total_point_amount' => 4132,         // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
        'applicable_account_metadata' => ["key" => "sex", "value" => "male"]
,                                                 // ウォレットに紐付くメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
        'applicable_transaction_metadata' => ["key" => "rank", "value" => "bronze"]
,                                                 // 取引時に指定するメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
        'budget_caps_amount' => 1167364432        // キャンペーン予算上限
    ]
);
```
