...
$request = new Request\GetCpmToken(
    "qMOfry8huKLaun9q8fRCMt"                      // cpmToken: CPMトークン
);
```
...
$request = new Request\ListTransactions(
    [
        'from' => "2022-03-09T03:54:08.000000Z",  // 開始日時
        'to' => "2022-04-03T06:26:58.000000Z",    // 終了日時
        'page' => 1,                              // ページ番号
...
$request = new Request\CreateTransaction(
    ...
    [
        'money_amount' => 4953,
        'point_amount' => 5261,
        'point_expires_at' => "2022-01-26T10:35:17.000000Z", // ポイント有効期限
        'description' => "awpUouvYHKlj0GUL0Fcnz7fEngR6pF3m54VmwYrgFgT3RyUt1Kexb2ZIYN08OgDDQYpUk9QvTpwbva3X3fUufQzzx2hzebS68SpNEGkfmS3U"
    ]
);
```
...
$request = new Request\ListTransactionsV2(
    [
        ...
        'types' => ["topup", "payment"],          // 取引種別 (複数指定可)、チャージ=topup、支払い=payment
        'from' => "2023-10-02T12:06:13.000000Z",  // 開始日時
        'to' => "2023-02-17T21:38:34.000000Z",    // 終了日時
...
$request = new Request\CreateTopupTransaction(
    ...
    [
        ...
        'money_amount' => 4401,                   // マネー額
        'point_amount' => 6958,                   // ポイント額
        'point_expires_at' => "2020-04-03T09:24:38.000000Z", // ポイント有効期限
...
$request = new Request\CreateCpmTransaction(
    "KUg3om1YNfeeKoLdFE8Hmt",                     // cpmToken: CPMトークン
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",       // shopId: 店舗ID
    5842.0,                                       // amount: 取引金額
    [
...
$request = new Request\CreateExchangeTransaction(
    ...
    2969,
    [
        'description' => "v1AJsBz3l6W699PQnfTErfIkmiU4i2bFcYt3zvnnQAgg6WKGNaTc3A08bOic61u1yVQPNCQEFIkbwhO9RJiR7mxn7kYGzShazSiZH6DDfNqfsVRi3zxzsVzVJLxpF9uCjOUSNMH9fWh27PiOpr",
        'request_id' => "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" // リクエストID
    ]
);
```
...
$request = new Request\RefundTransaction(
    ...
    [
        ...
        'returning_point_expires_at' => "2020-11-05T22:22:56.000000Z" // 返却ポイントの有効期限
    ]
);
```
